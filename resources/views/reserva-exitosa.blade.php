@extends('layouts.app')
@section('title', __('Reserva Exitosa'))
    ...
    <div class="reservation-card text-center" style="padding: 4rem 2rem; box-shadow: 0 20px 50px rgba(0,0,0,0.1); border: none;">
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 1rem; padding: 2rem; text-align: left; margin-bottom: 3rem;">
            <div style="display: flex; gap: 1.5rem; align-items: center; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px dashed #cbd5e1;">
                <img src="{{ $alquiler->auto->imagen_url }}" style="width: 120px; border-radius: 0.5rem;" alt="Auto">
                <div>
                    <h4 style="margin: 0; font-size: 1.25rem;">{{ $alquiler->auto->modelo->nombre_modelo }}</h4>
                    <p style="margin: 0; color: #64748b;">{{ $alquiler->auto->modelo->marca->nombre_marca }} — {{ $alquiler->auto->anio }}</p>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div>
                    ...
                    <p style="margin: 0; font-weight: 600; color: #1e293b;">{{ $alquiler->fecha_retiro->format('d/m/Y') }}</p>
                </div>
                <div>
                    ...
                    <p style="margin: 0; font-weight: 600; color: #1e293b;">{{ $alquiler->fecha_devolucion->format('d/m/Y') }}</p>
                </div>
                <div style="grid-column: 1 / -1; background: #fff; padding: 1rem; border-radius: 0.5rem; display: flex; justify-content: space-between; align-items: center; border: 1px solid #f1f5f9;">
                    ...
                    <span style="font-size: 1.5rem; font-weight: 800; color: #2563eb;">${{ number_format($alquiler->precioTotal, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>

        <div style="display: flex; gap: 1rem; justify-content: center;">
            <a href="{{ route('catalogo') }}" class="btn btn-outline" style="padding: 0.75rem 2rem;">{{ __('Volver al Catálogo') }}</a>
            <button onclick="window.print()" class="btn btn-primary" style="padding: 0.75rem 2rem;">
                <i class="fa-solid fa-print" style="margin-right: 0.5rem;"></i> {{ __('Imprimir Comprobante') }}
            </button>
        </div>
    </div>
</div>
